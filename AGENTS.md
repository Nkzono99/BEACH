...
- `make test` は開発用 L1 とし、重い FMM 系（`test_dynamics_fmm`, `test_coulomb_fmm_core_basic`）は `make test-l3` / `make test-heavy` / `make test-fortran-heavy` / `make test-full` で明示実行する。`m2l_root_oracle` far-correction 診断（`test_coulomb_fmm_core_periodic`, `test_periodic2_flat_oracle_diag`）はさらに重いため `make test-fortran-far-correction` または `make test-full` で opt-in 実行する。新しい長時間 FMM 回帰テストは、`make test` の既定経路に足す前に opt-in の L3/heavy へ分離する。
