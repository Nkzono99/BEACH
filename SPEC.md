...
近傍画像和は `sim.field_periodic_image_layers = N` に対して各周期軸 `[-N, N]` を評価します。`sim.field_periodic_far_correction="ewald_like"` の場合はその外側を erfc スクリーン核で近似補正し、`"ewald"` の場合は 2D periodic Ewald 和の実空間項・逆空間項・`k=0` 項で補正します。`"m2l_root"` の場合は、近傍画像殻の外側を root periodic M2L 演算子として plan 構築時に前計算し、runtime では root local へ注入して既存 L2L に流します。`m2l_root` の非中性ケースは `charged_walls` の規約に合わせ、発散する monopole potential mode を gauge として落とした field-only closure を使います（`field_periodic_ewald_layers` はこのモードでは外側画像殻の打切り深さとして使い、`field_periodic_ewald_alpha` は未使用）。
