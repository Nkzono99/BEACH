...
近傍画像和は `sim.field_periodic_image_layers = N` に対して各周期軸 `[-N, N]` を評価します。現行の `periodic2` 遠方補正は正規名 `m2l_root_trunc` のみで、近傍画像殻の外側を truncated far-image root operator として plan 構築時に前計算し、runtime では root local へ注入して既存 L2L に流します。`sim.field_periodic_far_correction="none"` は後方互換のため受理しますが、`field_solver="fmm"` かつ `field_bc_mode="periodic2"` では内部的に `m2l_root_trunc` に正規化されます。legacy な `m2l_root` も `m2l_root_trunc` の alias として受理します。現行実装は monopole source column を落とした neutral-residual truncation であり exact Ewald ではありません。非中性ケースは `charged_walls` の規約に合わせ、発散する monopole potential mode を gauge として落とした field-only closure として扱います（`field_periodic_ewald_layers` はこのモードでは外側画像殻の打切り深さとして使い、`field_periodic_ewald_alpha` は未使用）。
