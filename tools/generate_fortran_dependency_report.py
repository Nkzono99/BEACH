...
def markdown_report(entities: list[Entity], lang: str) -> str:
    ...
    labels = {
        "ja": {
            ...
            "lang": "Lang: [日本語](FortranDependencyMap.md) | [English](FortranDependencyMap.en.md)",
            "generated": "> このページは `tools/generate_fortran_dependency_report.py` から自動生成しています。",
            "overview": "概要",
            "source_files": "ソースファイル数",
            "modules": "モジュール数",
            "submodules": "submodule 数",
            "programs": "program 数",
            "internal_edges": "内部依存エッジ数",
            "graph": "全体グラフ",
            "graph_alt": "Fortran モジュール依存グラフ",
            "graph_note": "実線は `use` 依存、破線は `submodule(parent)` の親参照を表します。",
            "group_summary": "ディレクトリ別サマリ",
            "group": "ディレクトリ",
            "entity_count": "エンティティ数",
            "internal_dep_count": "内部依存数",
            "incoming_modules": "被依存の多いモジュール",
            "entity": "エンティティ",
            "incoming": "被依存数",
            "entity_list": "エンティティ一覧",
            "path": "パス",
            "internal_deps": "内部依存",
            "summary": "概要",
            "details": "詳細",
        ...
        "en": {
            ...
            "lang": "Lang: [English](FortranDependencyMap.en.md) | [日本語](FortranDependencyMap.md)",
            "generated": "> This page is generated by `tools/generate_fortran_dependency_report.py`. Entity summaries are copied from Fortran source comments.",
            "overview": "Overview",
            "source_files": "Source files",
            "modules": "Modules",
            "submodules": "Submodules",
            "programs": "Programs",
            "internal_edges": "Internal dependency edges",
            "graph": "Overall Graph",
            "graph_alt": "Fortran module dependency graph",
            "graph_note": "Solid edges are `use` dependencies. Dashed edges are parent references from `submodule(parent)`.",
            "group_summary": "Summary by Directory",
            "group": "Directory",
            "entity_count": "Entities",
            "internal_dep_count": "Internal dependencies",
            "incoming_modules": "Most Depended-on Modules",
            "entity": "Entity",
            "incoming": "Incoming dependencies",
    ...
    lines: list[str] = []
    lines.append(f"title: {labels['title']}")
    lines.append("")
    lines.append(labels["lang"])
    lines.append("")
    lines.append(f"# {labels['title']}")
    lines.append("")
    lines.append(labels["generated"])
    lines.append("")
    lines.append(f"## {labels['overview']}")
    lines.append("")
    lines.append(f"- {labels['source_files']}: {file_count}")
    lines.append(f"- {labels['modules']}: {module_count}")
    lines.append(f"- {labels['submodules']}: {submodule_count}")
    lines.append(f"- {labels['programs']}: {program_count}")
    lines.append(f"- {labels['internal_edges']}: {internal_edge_count}")
    lines.append("")
    lines.append(f"## {labels['graph']}")
    lines.append("")
    lines.append(f"![{labels['graph_alt']}](../media/fortran_module_dependencies.svg)")
    lines.append("")
    lines.append(labels["graph_note"])
    lines.append("")
    lines.append(f"## {labels['group_summary']}")
    lines.append("")
    lines.append(
        f"| {labels['group']} | {labels['entity_count']} | {labels['internal_dep_count']} |"
    )
    lines.append("| --- | ---: | ---: |")
    ...
    lines.append("")
    lines.append(f"## {labels['incoming_modules']}")
    lines.append("")
    lines.append(f"| {labels['entity']} | kind | {labels['incoming']} |")
    lines.append("| --- | --- | ---: |")
    ...
    lines.append("")
    lines.append(f"## {labels['entity_list']}")
    lines.append("")
    lines.append(
        f"| {labels['entity']} | kind | {labels['path']} | {labels['internal_deps']} | {labels['summary']} |"
    )
    lines.append("| --- | --- | --- | --- | --- |")
    for entity in entities:
    ...
    for entity in entities:
        ...
        internal = (
            ", ".join(f"`{name}`" for name in entity.internal_dependencies)
            or labels["none"]
        )
        lines.append(f"- {labels['internal_deps']}: {internal}")
        external = ", ".join(
            f"`{name}`" for name in external_dependencies(entities, entity.path)
        ) or labels["none"]
        lines.append(f"- {labels['external_deps']}: {external}")
        if entity.summary:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Path to the generated Japanese Markdown page.",
    )
    parser.add_argument(
        "--markdown-en",
        type=Path,
        default=REPO_ROOT / "docs" / "FortranDependencyMap.en.md",
        help="Path to the generated English Markdown page.",
    )
    parser.add_argument(
        "--dot",
        type=Path,
...
def main() -> int:
    ...
    markdown_text = markdown_report(entities, lang="ja")
    markdown_text_en = markdown_report(entities, lang="en")

    write_text(args.dot, dot_text)
    render_svg(dot_text, args.svg)
    write_text(args.markdown, markdown_text)
    write_text(args.markdown_en, markdown_text_en)
    return 0
