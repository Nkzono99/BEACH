...
After this jitter, periodic axes are wrapped into the primitive cell and non-periodic axes are clamped to the box so every particle starts from a valid box position.
...
first face. A collision absorbs the particle without saving the candidate state. A third box event in the remainder fails
