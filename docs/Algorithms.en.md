...
| 7 | Open ends at the event; reflect/periodic rebuilds remainders for at most three events and checks each chord for mesh hits |
| 8 | Deposit `q * w` on a hit; fail closed without committing state at a fourth box event |
...
preserving mesh/box event ordering. If the next remainder reaches a fourth face without an earlier mesh hit, it returns
