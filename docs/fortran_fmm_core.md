...
- `periodic_far_correction`: `none`, `ewald_like`, `ewald`, `m2l_root`
- `periodic_ewald_alpha`, `periodic_ewald_layers`: `ewald_like` / `ewald` 補正の係数。`m2l_root` では `periodic_ewald_layers` を root periodic operator の外側画像殻深さに使い、`periodic_ewald_alpha` は未使用
...
`m2l_root` は free-space の P2M / M2M / M2L / L2L / L2P をそのまま使い、近傍画像殻の外側だけを root periodic M2L operator として前計算して root local へ足すモードです。

- `periodic_image_layers = N`: explicit に残す近傍画像殻
- `periodic_ewald_layers = L`: root periodic operator に畳み込む外側画像殻 `N < max(|i|,|j|) <= N+L`
- runtime では `local(:, root) += T_per_root * multipole(:, root)` を加えた後に既存 L2L を実行
- 非中性ケースは `charged_walls` の規約に合わせ、発散する periodic monopole potential mode を gauge として落とす field-only closure を使う
