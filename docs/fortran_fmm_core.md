...
- `periodic_far_correction`: `none`, `m2l_root_trunc`, `m2l_root_oracle`（legacy alias `m2l_root` も受理）
- `periodic_ewald_alpha`, `periodic_ewald_layers`: `m2l_root_oracle` の build-time exact Ewald oracle で使う分解パラメータと打切り深さ。`m2l_root_trunc` では `periodic_ewald_layers` を truncated root operator に畳み込む外側画像殻深さとして使う
...
eval_point(r):
  ...
  if leaf not found:
    ...
    add truncated far-image correction if needed
    return

  evaluate local expansion at leaf center
  add near direct interactions
  root local already carries truncated far-image correction when enabled
```
...
- current implementation は monopole source column を丸ごと落としており、意味としては exact periodic operator ではなく neutral-residual truncation に近い
- 非中性ケースでは `charged_walls` の規約に合わせ、発散する monopole potential mode を gauge として落とした field-only closure として使う
- BEACH adapter では `sim.field_bc_mode="periodic2"` のとき `field_periodic_far_correction="none"` を後方互換として受理し、内部では `m2l_root_trunc` に正規化する。legacy な `m2l_root` も同じく `m2l_root_trunc` として扱う
...
- build では proxy point charge basis から exact residual field を check points で評価し、root local coefficient へ least-squares fit した後に multipole basis へ戻す
- runtime では `local(:, root) += T_root_oracle * multipole(:, root)` を加えるだけなので eval path に Ewald 本体は入らない
- tree 外 fallback だけは exact periodic correction を直接足して、principal target box 外でも periodic residual を落とさない
- 現状の fit は potential ではなく field を使っており、local の定数 potential mode は gauge として 0 に固定する
...
- far correction は `none`, `m2l_root_trunc`, `m2l_root_oracle`（legacy alias `m2l_root` も受理）
