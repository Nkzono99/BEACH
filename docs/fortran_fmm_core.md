...
- `periodic_far_correction`: core が受ける値は `auto`, legacy `none`, `m2l_root_trunc`, `m2l_root_oracle`。`periodic2` 有効時の `auto` / legacy `none` は `m2l_root_oracle` に正規化される
...
legacy な `m2l_root` は BEACH adapter 側で `m2l_root_trunc` に正規化され、`periodic2` の `auto` / legacy `none` は `m2l_root_oracle` に正規化されます。
...
- BEACH adapter では `sim.field_bc_mode="periodic2"` のとき `field_periodic_far_correction="auto"` を既定として受理し、legacy な `none` も互換入力として受理して、内部では `m2l_root_oracle` に正規化する。legacy な `m2l_root` も adapter 側で `m2l_root_trunc` に正規化される
...
### 8.3 `periodic2` Ewald（Ewald2P）補正
...
- far correction は `auto`（既定、legacy `none` も受理）, `m2l_root_trunc`, `m2l_root_oracle`（`periodic2` の default `auto` は oracle に正規化）
