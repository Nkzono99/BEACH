...
- `periodic_far_correction`: `none`, `m2l_root_trunc`（legacy alias `m2l_root` も受理）
- `periodic_ewald_alpha`, `periodic_ewald_layers`: `periodic_ewald_alpha` は予約キーで未使用。`periodic_ewald_layers` は `m2l_root_trunc` の truncated root operator に畳み込む外側画像殻深さ
...
- far correction は `none`, `m2l_root_trunc`（legacy alias `m2l_root` も受理）
