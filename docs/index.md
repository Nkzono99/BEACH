...
このセクションでは、BEACH の利用者向け導線、設定ワークフロー、Fortran 実行系の運用、アルゴリズム解説、設定仕様、依存関係マップ、Python 後処理 API をまとめます。
FMM コア詳細と `batch_duration` の安定性は [BEACH アルゴリズム解説](Algorithms.html) に含めています。
...
API 単位の詳細は、FORD が生成する `Modules` / `Procedures` / `Types` 一覧から辿れます。
