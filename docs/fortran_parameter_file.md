...
| `field_solver` | string | `"auto"` | 電場評価方式。`direct` / `treecode` / `fmm` / `auto`（`auto` は要素数で方式・treecodeパラメータを自動選択） |
| `tree_theta` | float | `0.5` | treecode/FMM の MAC パラメータ（`0 < theta <= 1`、`field_solver="treecode"` または `"fmm"` 時に使用） |
| `tree_leaf_max` | int | `16` | treecode/FMM の葉ノードあたり最大要素数（`field_solver="treecode"` または `"fmm"` 時に使用） |
