...
| `far_correction` | `str` | - | `"none"` | `"auto"` / `"none"` / historical-read-only `"m2l_root_oracle"`. `auto` is treated as `"none"` for compatibility; the oracle residual itself is not reproduced |
...
- Removed `m2l_root_oracle` metadata is accepted only when reading historical outputs. Python does not reproduce its Ewald correction, and it does not affect direct-sum calculations
