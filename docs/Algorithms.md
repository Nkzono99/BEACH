...
| 7 | openはevent点で終了し、reflect/periodicは残り時間を最大3回再積分して各chordのmesh hitを調べる |
| 8 | hitなら`q * w`を堆積して吸収し、4回目のbox eventならstateをcommitせずfail closedにする |
...
4つ目のfaceまでにmesh hitがなければ、上限なしのloopへ入らず `particle_step_multiple_box_events` を返します。
