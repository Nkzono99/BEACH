program test_periodic2_operator_cache_mpi
  ...
  real(dp) :: local_target_min(1), local_target_max(1), qr_count_min(1), qr_count_max(1)
  integer(i32) :: global_build_count, global_cache_hits, global_local_target_count, global_qr_count
  ...
  global_qr_count = cold_plan%periodic_qr_preparation_count
  call mpi_allreduce_sum_i32_scalar(mpi, global_local_target_count)
  call mpi_allreduce_sum_i32_scalar(mpi, global_qr_count)
  ...
  call assert_equal_i32( &
    global_qr_count, cold_plan%periodic_root_target_count, &
    'cold MPI QR work must prepare each target exactly once' &
    )
  local_target_min(1) = real(cold_plan%periodic_operator_local_target_count, dp)
  local_target_max = local_target_min
  qr_count_min(1) = real(cold_plan%periodic_qr_preparation_count, dp)
  qr_count_max = qr_count_min
  call mpi_allreduce_min_real_dp_array(mpi, local_target_min)
  call mpi_allreduce_max_real_dp_array(mpi, local_target_max)
  call mpi_allreduce_min_real_dp_array(mpi, qr_count_min)
  call mpi_allreduce_max_real_dp_array(mpi, qr_count_max)
  call assert_true(local_target_max(1) - local_target_min(1) <= 1.0_dp, 'cold MPI target work is imbalanced')
  call assert_true(qr_count_max(1) - qr_count_min(1) <= 1.0_dp, 'cold MPI QR work is imbalanced')
  ...
  call assert_equal_i32(warm_plan%periodic_qr_preparation_count, 0_i32, 'warm cache QR preparation count')
