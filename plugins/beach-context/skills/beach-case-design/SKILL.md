...
description: Design BEACH simulation configs, parameter edits, and sweeps from physical intent while mapping choices to beach.toml, mesh, species, solver, output, and workload controls.
---

Use this skill when a user asks what parameter controls a phenomenon, how to build a new BEACH setup, or how to plan a parameter sweep.

## Response Language

- Respond in the user's language. If the request mixes languages and includes Japanese, use Japanese.
- Keep code identifiers, TOML keys, commands, and file paths unchanged.
