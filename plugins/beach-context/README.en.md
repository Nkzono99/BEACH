...
This repo-local plugin gives Codex BEACH-specific context for configuration files, run workflows, output analysis, known failure modes, simulator guidance, and feedback reporting. Skills are expected to respond in the user's language, including Japanese and English, while keeping code identifiers, commands, file names, and parameter names unchanged.
