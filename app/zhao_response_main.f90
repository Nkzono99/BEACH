!> Zhao online evaluatorからmatching-plane response tableを生成するCLI。
program zhao_response_main
  ...
  if (command_argument_count() /= 3) then
    call print_usage()
    error stop 'beach-zhao-response requires exactly three positional arguments.'
  end if
  ...
  if (status /= matching_plane_generator_ok) error stop trim(message)
  print '(a,a)', 'matching-plane Zhao response written to ', trim(output_path)

contains

  subroutine print_usage()
    print '(a)', 'usage: beach-zhao-response <beach.toml> <query-grid.csv> <response.csv>'
  end subroutine print_usage

end program zhao_response_main
