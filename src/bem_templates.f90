  ...
  subroutine make_box(mesh, size, center, nx, ny, nz)
    ...
    do iz = 1, 2
      ...
      do ix = 0, nx0 - 1
        do iy = 0, ny0 - 1
          if (iz == 1) then
            call push_tri(v0,v1,v2,itri, [c(1)-hx+dx*ix,c(2)-hy+dy*iy,z], [c(1)-hx+dx*(ix+1),c(2)-hy+dy*(iy+1),z], [c(1)-hx+dx*(ix+1),c(2)-hy+dy*iy,z])
            call push_tri(v0,v1,v2,itri, [c(1)-hx+dx*ix,c(2)-hy+dy*iy,z], [c(1)-hx+dx*ix,c(2)-hy+dy*(iy+1),z], [c(1)-hx+dx*(ix+1),c(2)-hy+dy*(iy+1),z])
          else
            call push_tri(v0,v1,v2,itri, [c(1)-hx+dx*ix,c(2)-hy+dy*iy,z], [c(1)-hx+dx*(ix+1),c(2)-hy+dy*iy,z], [c(1)-hx+dx*(ix+1),c(2)-hy+dy*(iy+1),z])
            call push_tri(v0,v1,v2,itri, [c(1)-hx+dx*ix,c(2)-hy+dy*iy,z], [c(1)-hx+dx*(ix+1),c(2)-hy+dy*(iy+1),z], [c(1)-hx+dx*ix,c(2)-hy+dy*(iy+1),z])
          end if
        end do
      end do
    end do
    ...
    do ix = 1, 2
      ...
      do iy = 0, ny0 - 1
        do iz = 0, nz0 - 1
          if (ix == 1) then
            call push_tri(v0,v1,v2,itri, [x,c(2)-hy+dy*iy,c(3)-hz+dz*iz], [x,c(2)-hy+dy*(iy+1),c(3)-hz+dz*(iz+1)], [x,c(2)-hy+dy*(iy+1),c(3)-hz+dz*iz])
            call push_tri(v0,v1,v2,itri, [x,c(2)-hy+dy*iy,c(3)-hz+dz*iz], [x,c(2)-hy+dy*iy,c(3)-hz+dz*(iz+1)], [x,c(2)-hy+dy*(iy+1),c(3)-hz+dz*(iz+1)])
          else
            call push_tri(v0,v1,v2,itri, [x,c(2)-hy+dy*iy,c(3)-hz+dz*iz], [x,c(2)-hy+dy*(iy+1),c(3)-hz+dz*iz], [x,c(2)-hy+dy*(iy+1),c(3)-hz+dz*(iz+1)])
            call push_tri(v0,v1,v2,itri, [x,c(2)-hy+dy*iy,c(3)-hz+dz*iz], [x,c(2)-hy+dy*(iy+1),c(3)-hz+dz*(iz+1)], [x,c(2)-hy+dy*iy,c(3)-hz+dz*(iz+1)])
          end if
        end do
      end do
    end do
    ...
    do iy = 1, 2
      ...
      do ix = 0, nx0 - 1
        do iz = 0, nz0 - 1
          if (iy == 1) then
            call push_tri(v0,v1,v2,itri, [c(1)-hx+dx*ix,y,c(3)-hz+dz*iz], [c(1)-hx+dx*(ix+1),y,c(3)-hz+dz*iz], [c(1)-hx+dx*(ix+1),y,c(3)-hz+dz*(iz+1)])
            call push_tri(v0,v1,v2,itri, [c(1)-hx+dx*ix,y,c(3)-hz+dz*iz], [c(1)-hx+dx*(ix+1),y,c(3)-hz+dz*(iz+1)], [c(1)-hx+dx*ix,y,c(3)-hz+dz*(iz+1)])
          else
            call push_tri(v0,v1,v2,itri, [c(1)-hx+dx*ix,y,c(3)-hz+dz*iz], [c(1)-hx+dx*(ix+1),y,c(3)-hz+dz*(iz+1)], [c(1)-hx+dx*(ix+1),y,c(3)-hz+dz*iz])
            call push_tri(v0,v1,v2,itri, [c(1)-hx+dx*ix,y,c(3)-hz+dz*iz], [c(1)-hx+dx*ix,y,c(3)-hz+dz*(iz+1)], [c(1)-hx+dx*(ix+1),y,c(3)-hz+dz*(iz+1)])
          end if
        end do
      end do
    end do
  ...
  subroutine make_cylinder(mesh, radius, height, n_theta, n_z, cap, center)
    ...
    do iz = 0, nz0 - 1
      ...
      do it = 0, nt - 1
        ...
        call push_tri(v0,v1,v2,itri, [c(1)+r*cos(t0),c(2)+r*sin(t0),z0], [c(1)+r*cos(t1),c(2)+r*sin(t1),z0], [c(1)+r*cos(t1),c(2)+r*sin(t1),z1])
        call push_tri(v0,v1,v2,itri, [c(1)+r*cos(t0),c(2)+r*sin(t0),z0], [c(1)+r*cos(t1),c(2)+r*sin(t1),z1], [c(1)+r*cos(t0),c(2)+r*sin(t0),z1])
      end do
    end do
    ...
    if (cap0) then
      do it = 0, nt - 1
        t0 = 2.0d0 * pi * real(it, dp) / real(nt, dp)
        t1 = 2.0d0 * pi * real(mod(it + 1, nt), dp) / real(nt, dp)
        call push_tri(v0,v1,v2,itri, [c(1),c(2),c(3)-0.5d0*h], [c(1)+r*cos(t1),c(2)+r*sin(t1),c(3)-0.5d0*h], [c(1)+r*cos(t0),c(2)+r*sin(t0),c(3)-0.5d0*h])
        call push_tri(v0,v1,v2,itri, [c(1),c(2),c(3)+0.5d0*h], [c(1)+r*cos(t0),c(2)+r*sin(t0),c(3)+0.5d0*h], [c(1)+r*cos(t1),c(2)+r*sin(t1),c(3)+0.5d0*h])
      end do
    end if
