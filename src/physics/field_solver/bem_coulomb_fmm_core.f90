!> `mesh_type` や `sim_config` に依存しない Coulomb FMM コア。
...
contains
  ...
  subroutine build_plan(plan, src_pos, options)
  ...
  subroutine update_state(plan, state, src_q)
    ...
    if (plan%nsrc > 0_i32) state%src_q = src_q
    if (allocated(state%multipole)) state%multipole = 0.0d0
    if (allocated(state%local)) state%local = 0.0d0
  ...
  subroutine eval_points(plan, state, target_pos, e)
    type(fmm_plan_type), intent(in) :: plan
  ...
  subroutine p2m_leaf_moments(plan, state)
    type(fmm_plan_type), intent(in) :: plan
    type(fmm_state_type), intent(inout) :: state
    integer(i32) :: node_idx, p, idx, p_end, alpha_idx
    real(dp) :: d(3)
    real(dp) :: xpow(0:max(0_i32, plan%options%order)), ypow(0:max(0_i32, plan%options%order))
    ...
    do node_idx = 1_i32, plan%nnode
      ...
      do p = plan%node_start(node_idx), p_end
        ...
        do alpha_idx = 1_i32, plan%ncoef
          state%multipole(alpha_idx, node_idx) = state%multipole(alpha_idx, node_idx) + state%src_q(idx) &
    ...
    do depth = plan%node_max_depth - 1_i32, 0_i32, -1_i32
      ...
      do level_pos = level_start_pos, level_end_pos
        ...
        do child_k = 1_i32, plan%child_count(node_idx)
          ...
          d = plan%node_center(:, child_node) - plan%node_center(:, node_idx)
          call build_axis_powers(d, plan%options%order, xpow, ypow, zpow)
          do beta_idx = 1_i32, plan%ncoef
            do alpha_idx = 1_i32, plan%ncoef
              if (any(plan%alpha(:, alpha_idx) > plan%alpha(:, beta_idx))) cycle
              delta = plan%alpha(:, beta_idx) - plan%alpha(:, alpha_idx)
              delta_idx = plan%alpha_map(delta(1), delta(2), delta(3))
              shift_factor = xpow(delta(1)) * ypow(delta(2)) * zpow(delta(3)) / plan%alpha_factorial(delta_idx)
              state%multipole(beta_idx, node_idx) = state%multipole(beta_idx, node_idx) &
                                                    + state%multipole(alpha_idx, child_node) * shift_factor
            end do
          end do
        end do
      end do
      ...
      do pair_pos = plan%m2l_target_start(t_node), plan%m2l_target_start(t_node + 1_i32) - 1_i32
        ...
        do alpha_idx = 1_i32, plan%ncoef
          do beta_idx = 1_i32, plan%ncoef
            ...
            state%local(alpha_idx, t_node) = state%local(alpha_idx, t_node) + plan%alpha_sign(beta_idx) &
                                             * state%multipole(beta_idx, s_node) * plan%m2l_deriv(deriv_idx, pair_idx)
          end do
        end do
      end do
    end do
    ...
    do depth = 1_i32, max_depth
      ...
      do level_pos = level_start_pos, level_end_pos
        if (use_target_tree) then
          ...
          d = plan%target_node_center(:, node_idx) - plan%target_node_center(:, plan%parent_of(node_idx))
        else
          node_idx = plan%node_level_nodes(level_pos)
          d = plan%node_center(:, node_idx) - plan%node_center(:, plan%parent_of(node_idx))
        end if
        ...
        do alpha_idx = 1_i32, plan%ncoef
          do gamma_idx = 1_i32, plan%ncoef
            ...
            delta_idx = plan%alpha_map(delta(1), delta(2), delta(3))
            shift_factor = xpow(delta(1)) * ypow(delta(2)) * zpow(delta(3)) / plan%alpha_factorial(delta_idx)
            state%local(alpha_idx, node_idx) = state%local(alpha_idx, node_idx) + state%local(gamma_idx, parent_node) * shift_factor
          end do
        end do
      end do
      !$omp end parallel do
