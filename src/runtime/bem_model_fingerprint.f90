...
contains
  ...
  function model_fingerprint(cfg) result(fingerprint)
    ...
    call feed_integer(hash, cfg%outer_plasma%photoelectron_histogram_bins)
    call feed_real(hash, cfg%outer_plasma%photoelectron_histogram_energy_max)
    call feed_real(hash, cfg%outer_plasma%photoelectron_ambient_charge_scale)
    ...
    call feed_real(hash, cfg%coupling%outer_orbit_dt)
    call feed_integer(hash, cfg%coupling%outer_orbit_max_steps)
    call feed_real(hash, cfg%coupling%outer_orbit_energy_tolerance)
