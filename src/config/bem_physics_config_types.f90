...
contains
  ...
  !> 公開の solver option の値域を検証する。呼出元によらず同じ契約を使う。
  subroutine validate_field_solver_parameters(sim, status, message)
    type(sim_config), intent(in) :: sim
    integer(i32), intent(out) :: status
    character(len=*), intent(out) :: message
    ...
    status = physics_config_ok
    message = ''
    select case (trim(lower_ascii(sim%field_solver)))
    case ('direct', 'treecode', 'fmm', 'auto')
      continue
    case default
      call reject( &
        physics_config_invalid_combination, &
        'sim.field_solver must be "direct", "treecode", "fmm", or "auto".', status, message &
        )
      return
    end select
    select case (trim(lower_ascii(sim%field_normalization)))
    case ('si', 'box', 'mesh', 'length')
      continue
    case default
      call reject( &
        physics_config_invalid_combination, &
        'sim.field_normalization must be "si", "box", "mesh", or "length".', status, message &
        )
      return
    end select
    if (.not. ieee_is_finite(sim%field_length_scale) .or. sim%field_length_scale <= 0.0d0) then
      call reject( &
        physics_config_invalid_combination, &
        'sim.field_length_scale must be finite and > 0.', status, message &
        )
      return
    end if
    select case (trim(lower_ascii(sim%field_bc_mode)))
    case ('free', 'periodic2')
      continue
    case default
      call reject( &
        physics_config_invalid_combination, &
        'field_boundary.mode must be "free" or "periodic2".', status, message &
        )
      return
    end select
    select case (trim(lower_ascii(sim%field_periodic_far_correction)))
    case ('auto')
      continue
    case ('none')
      continue
    case ('m2l_root_oracle')
      call reject( &
        physics_config_invalid_combination, &
        'sim.field_periodic_far_correction="m2l_root_oracle" was removed; use "cached_kneq0".', status, message &
        )
      return
    case ('cached_kneq0')
      continue
    case default
      call reject( &
        physics_config_invalid_combination, &
        'sim.field_periodic_far_correction must be "auto", "none", or "cached_kneq0".', status, message &
        )
      return
    end select
    if (trim(lower_ascii(sim%field_periodic_far_correction)) == 'cached_kneq0') then
      if (trim(lower_ascii(sim%field_solver)) /= 'fmm' .or. trim(lower_ascii(sim%field_bc_mode)) /= 'periodic2') then
        call reject( &
          physics_config_invalid_combination, &
          'sim.field_periodic_far_correction requires sim.field_solver="fmm" and '// &
          'field_boundary.mode="periodic2".', status, message &
          )
        return
      end if
      if (sim%field_periodic_ewald_layers < 1_i32) then
        call reject( &
          physics_config_invalid_combination, &
          'sim.field_periodic_ewald_layers must be >= 1 when far correction is enabled.', status, message &
          )
        return
      end if
    end if
    if (trim(lower_ascii(sim%field_bc_mode)) == 'periodic2') then
      if (sim%field_periodic_image_layers < 0_i32) then
        call reject( &
          physics_config_invalid_combination, &
          'sim.field_periodic_image_layers must be >= 0 for periodic2.', status, message &
          )
        return
      end if
    end if
    if (trim(lower_ascii(sim%field_periodic_far_correction)) == 'cached_kneq0') then
      if (sim%field_periodic_image_layers < 1_i32) then
        call reject( &
          physics_config_invalid_combination, &
          'cached_kneq0 requires sim.field_periodic_image_layers >= 1.', status, message &
          )
        return
      end if
      if (.not. ieee_is_finite(sim%field_periodic_ewald_alpha) .or. sim%field_periodic_ewald_alpha < 0.0d0) then
        call reject( &
          physics_config_invalid_combination, &
          'sim.field_periodic_ewald_alpha must be finite and >= 0 for cached_kneq0.', status, message &
          )
        return
      end if
      if (len_trim(sim%field_periodic_cache_dir) == 0) then
        call reject( &
          physics_config_invalid_combination, &
          'sim.field_periodic_cache_dir must not be empty for cached_kneq0.', status, message &
          )
        return
      end if
      if (.not. ieee_is_finite(sim%field_periodic_generation_tolerance) .or. &
          sim%field_periodic_generation_tolerance <= 0.0_dp) then
        call reject( &
          physics_config_invalid_combination, &
          'sim.field_periodic_generation_tolerance must be finite and > 0 for cached_kneq0.', status, message &
          )
        return
      end if
    end if
    if (trim(lower_ascii(sim%field_solver)) /= 'direct') then
      if (.not. ieee_is_finite(sim%tree_theta) .or. sim%tree_theta <= 0.0d0 .or. sim%tree_theta > 1.0d0) then
        call reject( &
          physics_config_invalid_combination, &
          'sim.tree_theta must be finite and satisfy 0 < theta <= 1 for tree-capable solvers.', status, message &
          )
        return
      end if
      if (sim%tree_leaf_max < 1_i32) then
        call reject( &
          physics_config_invalid_combination, &
          'sim.tree_leaf_max must be >= 1 for tree-capable solvers.', status, message &
          )
        return
      end if
      if (sim%tree_min_nelem < 1_i32) then
        call reject( &
          physics_config_invalid_combination, &
          'sim.tree_min_nelem must be >= 1 for tree-capable solvers.', status, message &
          )
        return
      end if
    end if
  end subroutine validate_field_solver_parameters
  ...
  subroutine validate_active_physics_config(sim, field, periodic2, panel, status, message)
    ...
    call validate_field_solver_parameters(sim, status, message)
